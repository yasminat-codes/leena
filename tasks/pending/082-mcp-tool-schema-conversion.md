...
5. Add validation: reject tools with empty name, description longer than 1024 chars (OpenAI limit), or schema deeper than 5 levels of nesting (truncate with `additionalProperties: true` at the cutoff).

## Acceptance Criteria
- [ ] `mcpToolToOpenAI` converts a standard MCP tool definition to valid OpenAI function format
- [ ] `sanitizeSchema` removes `$schema`, `$id`, `$comment`, `examples`, `default` keys recursively
- [ ] `sanitizeSchema` adds `type: "object"` and `properties: {}` when missing from root
- [ ] `namespaceMCPTool` produces `mcp__{serverId}__{toolName}` format
- [ ] `parseMCPToolName` correctly reverses namespaced names; returns null for non-MCP names
- [ ] `getMergedToolDefinitions` merges static + MCP tools with no name collisions
- [ ] Disconnected or errored servers are skipped gracefully (their tools not included)
- [ ] Nested schemas deeper than 5 levels are truncated safely
